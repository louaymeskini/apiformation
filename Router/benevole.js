...
Router.get("/liste/evenement/:id", validateUser, function (req, res) {
    benevoleModel.find({_id: req.params.id}).select('evenements').populate({
        path: 'evenements',
        populate: {path: 'association'}
    }).exec(function (errr, result) {
        if (errr)
            res.send({"state": "not ok", "msg": "err:" + errr});
        else
            res.send(result);
...
//supprimer benevole (angular)
Router.delete("/supprimer/:id", validateUser, function (req, res) {
    associationModel.updateMany({benevoles: {$in: req.params.id}}, {$pull: {benevoles: req.params.id}}, function (err) {
    })
...
Router.put("/supprimer/:id/association/:idA", validateUser, function (req, res) {
    benevoleModel.findOneAndUpdate({_id: req.params.id}, {$pull: {associations: req.params.idA}}, function (err, resu) {
        if (err) {
            res.send({"state": "non", "msg": "err:" + err});
        }
        else {
            associationModel.findOneAndUpdate({_id: req.params.idA}, {$pull: {benevoles: req.params.id}}, function (err, result) {
                if (err) {
                    res.send({"state": "non", "msg": "err:" + err});
                } else {
                    ...
                    benevoleModel.findOneAndUpdate({_id: req.params.id}, {$pullAll: {annonces: result.annonces}}, function (err, resu) {
                    })
                    //associationModel.save();
...
//all association (oui + non + cours)
Router.get("/:id/association", validateUser, function (req, res) {
    associationModel.find({}, function (err, resu) {
        if (err) {
            res.send({"state": "non", "msg": "err:" + err});
        } else {
            benevoleModel.find({_id: req.params.id}).exec(function (errr, resul) {
                if (err) {
                    res.send({"state": "non", "msg": "err:" + err});
                }
                else {
                    ...
                    var non = [];
                    for (var x in resu) {
                        if (resul[0].associations.indexOf(resu[x]._id) != -1) {
                            ouiCours.push(resu[x]);
                        } else {
                            non.push(resu[x]);
                        }
                    }

                    for (var oc in ouiCours) {
                        var existInArray = false;
                        for (var benasso in ouiCours[oc].benevoles) {
                            // return res.send(ouiCours[oc].benevoles[benasso] == req.params.id);
                            if (ouiCours[oc].benevoles[benasso] == req.params.id) {
                                existInArray = true
                                // res.send(existInArray)
                            }
                        }
                        if (existInArray) {
                            oui.push(ouiCours[oc])
                        } else {
                            cours.push(ouiCours[oc])
                        }
                    }
                    res.send({"oui": oui, "cours": cours, "non": non});
                }
            })
